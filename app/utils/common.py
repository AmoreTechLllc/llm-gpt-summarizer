...
def is_valid_mastodon_url(url: str) -> bool:
    """
    Check if the URL is a valid Mastodon post URL.
    """
    pattern = re.compile(
        r"^"  # Start of the string
        r"(http(s)?:\/\/)?"  # Optional "http://" or "https://"
        r"(www\.)?"  # Optional "www."
        r"[^\/]+"  # Matches any characters except "/"
        r"\/@"  # Matches "/@"
        r"[a-zA-Z0-9_-]+\/"  # Matches the Mastodon handle followed by "/"
        r"[a-zA-Z0-9_]+"  # Matches one or more letters, numbers, or underscores (post ID or custom slug)
        r"\/?$"  # Optional "/" at the end of the string
    )
    return bool(pattern.match(url))
